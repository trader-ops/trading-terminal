            ...
            res.on('end', () => {
                try {
                    ...
                    if (!json.chart || !json.chart.result || !json.chart.result[0]) {
                        return reject(new Error(`Failed to parse chart data for ${symbol}: ${data.slice(0, 150)}`));
                    }
...
async function freezeDataset() {
    console.log('Downloading live 30-day historical data from Yahoo Finance...');
    ...
    try {
        console.log('1. Fetching Gold (GC=F) 5-minute candles (range=1mo)...');
        const gold5m = await fetchCandles('GC=F', '5m', '1mo');
        console.log(`✓ Fetched ${gold5m.length} Gold 5M candles. First: ${gold5m[0].iso}, Last: ${gold5m[gold5m.length - 1].iso}`);

        console.log('2. Fetching US Dollar Index (DX-Y.NYB) 15-minute candles (range=1mo)...');
        const dxy15m = await fetchCandles('DX-Y.NYB', '15m', '1mo');
        console.log(`✓ Fetched ${dxy15m.length} DXY 15M candles. First: ${dxy15m[0].iso}, Last: ${dxy15m[dxy15m.length - 1].iso}`);

        const payload = {
            metadata: {
                frozenAt: new Date().toISOString(),
                source: "Yahoo Finance Chart API (GC=F, DX-Y.NYB)",
                timeframeGold: "5m",
                timeframeDxy: "15m",
        ...
        const targetFile = path.join(dataDir, 'historical_candles_xauusd_dxy_30d.json');
        fs.writeFileSync(targetFile, JSON.stringify(payload, null, 2), 'utf8');
        const stats = fs.statSync(targetFile);
        console.log(`\n✅ DATASET FROZEN AND SAVED!`);
        console.log(`File: ${targetFile}`);
        console.log(`Size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`);
    } catch (err) {
        console.error('Error freezing dataset:', err);
        process.exit(1);
    }
}
